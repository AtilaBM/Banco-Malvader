...
void baixar_dados(const Conta *c);
...
int main(void)
{
    ...
    do
    {
        ...
        case 5:
        {
            ...
            {
                printf("Digite o CPF: ");
                if (fgets(c_cpf, TAM_CPF, stdin) == NULL)
                    break;
                c_cpf[strcspn(c_cpf, "\n")] = 0;
        ...
        case 9:
        {
            ...
            idx = encontrar_conta_por_numero(contas, qtd_contas, numero);

            if (idx != -1)
            {
                baixar_dados(&contas[idx]);
            }
            else
            {
                printf("\nERRO: Conta não encontrada no sistema.\n");
...
static int menu(void)
{
    ...
    printf("9. Baixar dados para arquivo\n");
...
int transferir(Conta contas[], int qtd, int origem, int destino, double valor)
{
    ...
    if (origem == destino)
        return -5;
    if (contas[indice_origem].status == ENCERRADA || contas[indice_destino].status == ENCERRADA)
        return -3;
    if (contas[indice_origem].saldo < valor)
        return -4;
...
void baixar_dados(const Conta *c)
{
    ...
    fprintf(arq, "Saldo: R$%.2lf\n", c->saldo);
    fprintf(arq, "---------------------------\n");

    fclose(arq);
    printf("Dados salvos em data/clientes.txt\n");
}